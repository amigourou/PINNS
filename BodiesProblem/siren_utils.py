...
from BodiesProblem.utils import save_plot

class LearnableSineCosActivation(nn.Module):
    """Learnable activation function: w1 * sin(x) + w2 * cos(x)."""
    def __init__(self):
        super(LearnableSineCosActivation, self).__init__()
        # Initialize learnable parameters w1 and w2
...
class SirenLayer(nn.Module):
    """A single SIREN layer with learnable sine-cos activation and specialized initialization."""
    def __init__(self, in_features, out_features, omega_0=30.0, is_first=False):
        ...
        # Use the learnable sine-cos activation
        self.activation = LearnableSineCosActivation()
    ...
    def forward(self, x):
        return self.activation(self.omega_0 * self.linear(x))


class SIREN(nn.Module):
    """Full SIREN model with a backbone and output layers."""
    def __init__(self, in_features, out_features, hidden_features, hidden_layers, omega_0=30.0):
...
class GravitationalBodiesSIREN(nn.Module):
    ...
    def __init__(self, system,
        ...
        self.uniform_times = torch.linspace(0, 1, self.num_physics_points).to(self.device)
    ...
    def compute_solution(self):
        with torch.no_grad():
            self.true_position, self.true_velocity,self.physics_times = self.system.get_solution() #self.physics_times_uniform.squeeze(1).cpu())
            # self.uniform_times = torch.linspace(0, 1, len(self.physics_times)).to(self.device)
            self.physics_times = torch.tensor(self.physics_times, device= self.device).unsqueeze(1).to(torch.float32)
            # plt.scatter(self.uniform_times.cpu(), self.physics_times.squeeze(1).cpu())
            # plt.show()
            print(self.physics_times.shape)
    ...
    def save_results(self, epoch, folder, train_data):
        ...
        save_plot(preds_r.T.cpu(), self.true_position.T.cpu(), self.physics_times.squeeze(1).cpu(),name=os.path.join(folder,"positions", f"position_{epoch}.png"), opt_scatter=train_data)
        save_plot(preds_v.T.cpu(), self.true_velocity.T.cpu(), self.physics_times.squeeze(1).cpu(),name=os.path.join(folder,"velocities", f"velocity_{epoch}.png"))
        save_plot(preds_a.T.cpu(), self.true_acceleration.T.cpu(), self.physics_times.squeeze(1).cpu(),name=os.path.join(folder,"accelerations", f"acceleration2_{epoch}.png"))
    ...
    def prepare_results(self):
        ...
        preds_a = torch.gradient(preds_v, spacing=(self.physics_times.squeeze(1),), dim=0)[0]
        # test_a_preds = torch.concat([test_a_preds, ground_truth_a[:,2:]], axis = 1)
        return preds_r.T.cpu(), preds_v.T.cpu(), preds_a.T.cpu(), self.physics_times.squeeze(1).cpu()
